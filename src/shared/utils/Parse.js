...
export class Parse extends Backend {
   ...
   */
	async signup(data) {
		return await this._fetch({method: 'POST', url: '/users', body: data}).then((res) => {
   ...
   */
	async logout() {
		return await this._fetch({method: 'POST', url: '/logout', body: {}}).then((res) => {
			if ((res.status === 200 || res.status === 201 || res.status === 400) || (res.code === 209)) {
				return {}
			} else {
   ...
   */
	async resetPassword(data) {
		return await this._fetch({method: 'POST', url: '/requestPasswordReset', body: data}).then((res) => {
			return res.json().then(function(json) {
				if ((res.status === 200 || res.status === 201)) {
					return {}
				} else {
   ...
   */
	async getProfile() {
		return await this._fetch({method: 'GET', url: '/users/me'}).then((response) => {
